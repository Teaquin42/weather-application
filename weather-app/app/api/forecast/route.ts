...
export async function GET(req: NextRequest) {
  ...
  const isPostcode = /^([A-Z][A-HJ-Y]?\d[A-Z\d]? ?\d[A-Z]{2}|GIR ?0A{2})$/.test(
    trimmedLocation
  );

  // Set url based on if location is city or UK postcode
  const url = isPostcode
    ? `https://api.openweathermap.org/data/2.5/forecast?zip=${trimmedLocation},GB&units=metric&appid=${apiKey}`
